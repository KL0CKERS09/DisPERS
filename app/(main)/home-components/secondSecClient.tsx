...
export default function SecondSection() {
  const [alerts, setAlerts] = useState<Alert[]>([]); // Use the Alert type here
  const [loading, setLoading] = useState(true);
  const [expanded, setExpanded] = useState(false);
  const [selectedAlert, setSelectedAlert] = useState<Alert | null>(null); // Use Alert | null for selectedAlert
  ...
  useEffect(() => {
    const fetchAlerts = async () => {
      try {
        ...
        setAlerts([]); // Reset alerts on error
      } finally {
        setLoading(false);
      }
    };
                ...
                >
                  ...
                  <div className="w-[80%] py-5">
                    ...
                    <div className="min-h-[3em] flex justify-between items-center">
                      ...
                      <button
                        onClick={() => setSelectedAlert(alert)} // Correct usage
